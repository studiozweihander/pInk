...
export class IssuesRepository {
  async findById(id: string): Promise<IssueDB | null> {
    const { data, error } = await supabase.from<IssueDB>("Issue").select("*").eq("id", id).single();
    if (error) handleSupabaseError(error, "issues.findById");
    return data ?? null;
  }

  async findByComic(comicId: string): Promise<IssueDB[]> {
    const { data, error } = await supabase.from<IssueDB>("Issue").select("*").eq("comic_id", comicId).order("number", { ascending: true });
    if (error) handleSupabaseError(error, "issues.findByComic");
    return data ?? [];
  }

  async create(payload: Partial<IssueDB>): Promise<IssueDB> {
    const { data, error } = await supabase.from<IssueDB>("Issue").insert([payload]).select().single();
