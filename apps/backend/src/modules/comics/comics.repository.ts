...
export class ComicsRepository {
  ...
  async findWithFilters(
    ...
    try {
      ...
      if (filters.publisherId) {
        query = query.eq("publisherId", filters.publisherId);
      }
      if (filters.idiomId) {
        query = query.eq("idiomId", filters.idiomId);
      }
      if (filters.year) {
  ...
  async getIdiomsByIds(ids: (number | null)[]): Promise<Map<number, Idiom>> {
    try {
      ...
      return new Map((data || []).map((idiom: Idiom) => [idiom.id, idiom]));
    } catch (error) {
  ...
  async getPublishersByIds(
    ids: (number | null)[]
  ): Promise<Map<number, Publisher>> {
    try {
      const validIds = [
        ...new Set(ids.filter((id): id is number => id !== null)),
      ];
      ...
      return new Map(
        (data || []).map((publisher) => [publisher.id, publisher])
      );
    } catch (error) {
