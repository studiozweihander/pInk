...
export class CardRenderer {
  ...
  createComicCard(comic) {
    ...
    const slug =
      comic.slug || this.generateSimpleSlug(comic.title || "comic-sem-titulo");

    return `
      <div class="card" data-id="${
        comic.id
      }" onclick="window.viewComicIssues('${slug}')">
  ...
  renderComics(comics) {
    ...
    this.cardsContainer.className = `cards has-content${
      appState.viewMode === VIEW_MODES.LIST ? ` ${CSS_CLASSES.listView}` : ""
    }`;
    this.cardsContainer.innerHTML = comics
      .map((comic) => this.createComicCard(comic))
      .join("");
  }
  ...
  renderIssues(issues) {
    ...
    this.cardsContainer.innerHTML = issues
  ...
  loadViewModePreference() {
    try {
      ...
      if (
        ...
        viewControls.setViewMode(savedMode);
      }
    } catch (e) {
      console.warn("Não foi possível carregar preferência de visualização:", e);
      const { viewControls } = require("./view-controls.js");
      viewControls.setViewMode(VIEW_MODES.GRID);
    }
  }
}
